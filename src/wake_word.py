...
def wake_word_matches(text: str, wake_word: str) -> bool:
    ...
    normalized_wake = re.sub(r"[^a-z0-9]+", "", wake_word.lower())
...
class WakeWordListener:
    ...
    def wait_for_utterance(self) -> Path:
        if self._can_use_pulse():
            return self._wait_with_pulse()
        return self._wait_with_sounddevice()

    def _can_use_pulse(self) -> bool:
        return bool(os.getenv("PULSE_SERVER")) and shutil.which("parecord") is not None

    def _wait_with_pulse(self) -> Path:
        command = [
            "parecord",
            "--record",
            "--raw",
        ...
        try:
            ...
            chunks = self._collect_speech_chunks(lambda: process.stdout.read(chunk_bytes))
    ...
    def _wait_with_sounddevice(self) -> Path:
        try:
            import numpy as np
    ...
    def _collect_speech_chunks(self, read_chunk) -> list[bytes]:
        chunks: list[bytes] = []
        in_speech = False
        speech_seconds = 0.0
        silent_seconds = 0.0
